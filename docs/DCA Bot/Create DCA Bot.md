...
|**trailing_deviation** | `boolean` | No | `true`<br>`false` | Percentage value of the trailing price, in percent. This parameter is required when `trailing_enabled` is set to `true` |
|**take_profit_type** | `string` | Yes | `base`<br>`total` | Take Profit order type for this DCA Bot |
