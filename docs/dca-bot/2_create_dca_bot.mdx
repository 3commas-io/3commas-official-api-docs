...
<ApiParam name='pairs' type='string' id="pairs" required>
   ...
   Use endpoint [All market pairs](/docs/market-data/1_all_market_pairs.mdx) to get value for parameter.
</ApiParam>
...
**leverage_custom_value**
   ...
   Use endpoints [Currency rates and limits with leverage data](/docs/market-data/3_currency_rates_and_limits_with_leverage_data.mdx) to check available value for this parameter.
...
The data strategies are used to populate the `strategy_list` and `close_strategy_list` parameters. You can retrieve the full list of strategies available for this DCA bot through the [Available strategy list for bot](/docs/dca-bot/1_available_strategy_list.mdx) endpoint.
...
<StickyLayout.Root>
   ...
   ## Response Parameter
      If successful, the response includes a update copy of [DCA Bot](/docs/dca-bot/0_dca_bot_entity.mdx) entity.
