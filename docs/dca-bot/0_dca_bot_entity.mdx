...
<CollapsibleApiParam name='custom_safety_steps' type='array[json]' id="custom_safety_steps">
   Direct list of custom SO steps (alternative to Martingale coefficients).
   Each step is JSON object: `{"deviation": "-1.1", "size": 10}`.

  <CollapsibleNestedParams>
     <ApiParam name='deviation' type="string" id="deviation">
       Step deviation in percents. Negative means lower than base price, positive is higher.

       <ParameterRange min={-100} max={10000} />
     </ApiParam>

     <ApiParam name='size' type="number" id="size">
       Size of averaging order on a given step. Interpretation of depends on `safety_order_volume_type` param.
     </ApiParam>
  </CollapsibleNestedParams>
</CollapsibleApiParam>
...
<CollapsibleApiParam name="strategy" type="string" id="strategy">
 Type of strategy (called Direction in UI) used by the bot: `long` or `short`.
</CollapsibleApiParam>
