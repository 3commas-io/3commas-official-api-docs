...
<Informer variant="warning">
  The endpoint is available for Deal for the statuses **bought** and
...
<ApiParam name="profit_currency" type="string" id="profit_currency" required>
  Profit currency used for calculating the profit from the trades executed by
  this DCA Bot: `quite_currency` or `base_currency`.
</ApiParam>
...
>
  Enables `true` or disable `false` the Move to Breakeven feature for this Deal.
...
<CollapsibleApiParam
  ...
  type="integer"
  id="active_safety_orders"
>
  The number of Safety Orders is allowed to place in advance on the exchange's
  order book.
</CollapsibleApiParam>
...
>
  The percentage by which the price needs to move in the opposite direction of
  your Take Profit target to trigger the Stop Loss, at which point the bot will
