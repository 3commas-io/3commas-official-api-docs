...
<ApiParam name="pairs" type="string" id="pairs" required>
  Trading pair(s) in 3Commas format. Use endpoint [All market
  pairs](/market-data/1_all_market_pairs.mdx) to get value for parameter.
</ApiParam>
...
>
  ...
  target that triggers the Stop Loss. The value should be set to `0` to disable
  Stop Loss
</CollapsibleApiParam>
...
>
  Value timeout in seconds. This parameter is required when
  `stop_loss_timeout_enabled` is set to `true`.
</CollapsibleApiParam>
...
>
  The minimum profit percentage. When it is reached, the deal will be completed
