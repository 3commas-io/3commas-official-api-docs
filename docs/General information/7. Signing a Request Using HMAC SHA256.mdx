...
<Steps>
  <Step title="Prepare the payload" description={<>Concatenate the query string and the request body to create the signature payload <code>totalParams</code>.
Ensure the data is formatted consistently.</>}
  />
  <Step title="Generate the binary signature" description= {<>Use the HMAC SHA256 algorithm with your secretKey as the key and totalParams as the data.</>}
  />
  <Step title="Encode the signature" description={<>Convert the binary output of the HMAC operation to a hexadecimal string.</>}
  />
</Steps>
...
<Steps>
  <Step title="Add the Signature to the request header" description={<>Include the computed signature in the Signature header of your request.</>}
  />
  <Step title="Include your API Key" description= {<>Add your 3Commas APIKey in the request header.</>}
  />
  <Step title="Send the request" description={<>Ensure the request method, headers, and payload are identical to those used during signature computation to avoid mismatches.
</>}
  />
</Steps>
...
<Tabs>
  ...
  <Step title="Request Preparation"
      description={
      <>
            <CodeBlock language="json" title="path" showLineNumbers>
            {`/public/api/ver1/users/change_mode`}
            </CodeBlock>
            <CodeBlock language="json" title="queryString" showLineNumbers>
            {`mode=paper`}
            </CodeBlock>
            <CodeBlock language="json" title="payload" showLineNumbers>
            {`/public/api/ver1/users/change_mode?mode=paper`}
            </CodeBlock>
            </>
            }
  />
<Step title="Generating the HMAC SHA256 Signature"
      description= {
      <>    
...
<Steps>
  <Step title="Request Preparation"
      ...
      <>
            ...
            <CodeBlock language="json" title="requestBody" showLineNumbers>
            {`mode=paper`}
            </CodeBlock>
            <CodeBlock language="json" title="payload" showLineNumbers>
            {`/public/api/ver1/users/change_mode?mode=paper`}
...
<Endpoint
  url="/public/api/ver1/bots/:bot_id/show"
  method="GET"
/>

<Steps>
  <Step title="Set up GET url"
        description={<>
        <p> With include_events <code>https://api.3commas.io/public/api/ver1/bots/EnterBotIdHere/show?include_events=true</code> </p> 
        <p> By using <code>include_events</code> in the query string, in Postman, your Params field will be automatically filled in.</p> 
        </>
        }
  />
  <Step title="Calculate your Signature" 
        description= {<>
  ...
  />
  <Step title="Receive JSON object"
        description={<>
