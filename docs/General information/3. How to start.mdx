...
<Steps>
  <Step title="Get Your 3Commas API Key" description={<>We offer two ways to generate API Key:<ul><li> • System generated;</li> <li> • Self-generated using an RSA asymmetric encryption method.</li></ul>
  We recommend the second method as it provides a more secure connection for your apps.</>}
  />
  <Step title="Create Signature" description= {<><code>SIGNED</code> endpoints require an additional <code>Signature</code> header to ensure the authenticity of the request.</>}
  />
  <Step title="Signed Endpoint" description={<>To authenticate, pass your 3Commas <code>API Key</code> and the <code>Signature</code> header in the request.</>}
  />
</Steps>
