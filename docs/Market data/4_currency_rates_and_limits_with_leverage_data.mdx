...
<ApiParam name='market_code' type='integer' id="market_code" required>
    ...
    Use endpoint [Supported markets list](/docs/market-data/0_supported_markets_list.mdx) to get value for parameter `market_code`.
</ApiParam>

<ApiParam name='pair' type='string' id="pair" required>
    Trading pair in 3Commas format. 

    Use endpoints [All Market Pairs](/docs/market-data/1_all_market_pairs.mdx) to get value for parameter `pair` in 3Commas format.
</ApiParam>
