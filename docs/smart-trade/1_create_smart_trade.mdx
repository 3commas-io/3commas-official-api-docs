...
<Informer variant="info">
  This endpoint supports creating SmartTrade is available for accounts where the
  parameter **smart_trading_supported** is **true**.
</Informer>
...
```json
{
  ...
  "error_attributes": {
    "position": ["is invalid"],
    "position.total": ["must be less than or equal to 9000000.0"],
    "units_to_buy": ["must be less than or equal to 9000.0"]
  }
}
```
