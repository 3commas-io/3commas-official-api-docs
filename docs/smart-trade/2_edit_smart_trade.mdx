...
<Informer variant="warning">
  Update SmartTrade is available for SmartTrade with status:
  **waiting_position** and **waiting_targets**.
</Informer>
...
<ApiParam name="leverage" type="object">
  ...
  <CollapsibleNestedParams>
    <ApiParam name="enabled" type="boolean" id="enabled" required>
      Enables leverage settings for this SmartTrade. Set to `true` to activate
      the parameter group.
    ...
    <ApiParam name="value" type="integer" id="value">
      Cross or isolated leverage value. Leverage value is dependent on the
      exchange. It should be based on [the rules of the exchange regarding
      currency rates, limits, and leverage
      data](/market-data/3_currency_rates_and_limits_with_leverage_data.mdx).
    </ApiParam>
  </CollapsibleNestedParams>
</ApiParam>
