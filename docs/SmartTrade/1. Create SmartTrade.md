...
|**leverage[value]**  | `integer` | No |  | Sets leverage settings for a futures account.<br>Default: `false` |
...
|**position[conditional][trailing][percent]**  | `number` | No | Minimum: `0`<br> Maximum: `99` | Percentage value of the trailing price for a conditional order. This parameter is required when `position[order_type]` is set to `conditional` |
...
|**take_profit[price_type]**| `string` | The price type for the take profit trade(s) that was set when creating the SmartTrade.<br>Can be either `value` or `percentage` |
