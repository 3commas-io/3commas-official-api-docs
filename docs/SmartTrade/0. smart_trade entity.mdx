  ...
  <CollapsibleNestedParams>
    ...
    <ApiParam name='reduce_funds_available' type="boolean" id="reduce_funds_available">
     ...
     <ApiParam name='force_start_available' type="boolean" id="force_start_available">
        <p>Indicates whether forcing a market entry is available (`true`) or not (`false`) for the SmartTrade, based on its current status.</p>
        <p>This option is available for SmartTrades when `position[status][basic_type]` is `to_process`, `order_placed`, `trailing_activated`</p>
    </ApiParam>
