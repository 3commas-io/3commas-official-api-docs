...
<Steps>
  <Step title="Prepare the payload" description= {
    <>
...
Ensure the data is formatted consistently.
  ...
  />
  <Step title="Generate the binary signature" description= {
    <>
    Use the HMAC SHA256 algorithm with your secretKey as the key and totalParams as the data.
    </>
    }
  />
  <Step title="Encode the signature" description={
    <>
...
<Steps>
  <Step title="Add the Signature to the request header" description= {
   <>
   Include the computed signature in the Signature header of your request.
   </>
   }
  />
  <Step title="Include your API Key" description= {
    <>
    Add your 3Commas APIKey in the request header.
    </>
  }
  />
  <Step title="Send the request" description={
    <>
...
**Look [here](https://developers-3commas.pages.dev/general-information/signing_a_request_using_hmac_sha256/generating-signature-hmac-sha256) for some examples.**
