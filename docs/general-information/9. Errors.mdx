...
* `4XX` Malformed requests (issue is on the sender's side);
* `429` Request rate limit exceeded;
* `418` IP auto-banned for continuing to send requests after receiving `429` codes;
* `5XX` Internal errors (issue is on 3Commas's side);
* `504` Gateway timeout, indicating the API couldn’t get a timely response.
