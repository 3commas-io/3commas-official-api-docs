...
<Steps>
  <Step
    ...
    description={
      <>
        Convert the data to be signed (signature payload) into _ASCII format_.
        This payload typically includes all relevant request parameters or
        fields.
      </>
    }
  />
  <Step
    ...
    description={
      <>
        Use the _RSASSA-PKCS1-v1_5_ signing algorithm with the SHA-256 hash
        function and _your RSA private key_ to create a binary signature.
      </>
    }
  />
  <Step
    ...
    description={
      <>
        Convert the binary output of the signature to a Base64 string (RFC
        2045).
      </>
    }
  />
...
<Steps>
  ...
  <Step
    ...
    description={
      <>
        Make the request to a SIGNED endpoint. Ensure the payload, headers, and
        method match the data used to compute the signature to avoid mismatches.
      </>
    }
  />
</Steps>
