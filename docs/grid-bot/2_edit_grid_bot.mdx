...
<ApiParam name='grids_quantity' type="number" id="grids_quantity" required>
  ...
  This value is limited based on [the exchange’s rules for rates, limits, and leverage](/docs/market-data/3_currency_rates_and_limits_with_leverage_data.mdx).
</ApiParam> 
...
If successful, the response includes a copy of the updated [Grid Bot](/docs/grid-bot/0_grid_bot_entity.mdx) entity.
</StickyLayout.Main>
