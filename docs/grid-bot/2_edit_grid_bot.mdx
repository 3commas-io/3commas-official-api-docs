...
>
  Indicates if upper stop-loss settings are enabled `true` or not `false`. Use
  `true` to activate the settings group.
</CollapsibleApiParam>
...
>
  Indicates if lower stop-loss settings are enabled `true` or not `false`. Use
  `true` to activate the settings group.
</CollapsibleApiParam>
...
<CollapsibleApiParam name="ignore_warnings" type="boolean" id="ignore_warnings">
  Ignores warnings and forces the creation of the Grid Bot `true` or not
  `false`.
</CollapsibleApiParam>
